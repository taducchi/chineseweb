...
// ═══════════════════════════════════════════════════════════════
// 🔊 HỆ THỐNG ÂM THANH (Web Audio API - không cần file mp3)
// ═══════════════════════════════════════════════════════════════
const useSoundEffects = () => {
    ...
    // Tiếng hoàn thành game (victory fanfare)
    const playVictory = () => {
...
// ─── COMPONENT ──────────────────────────────────────────────
export default function PracticeLesson({ course_slug, module_slug, lesson_slug }) {
    // ─── STATE ──────────────────────────────────────────────────
    ...
    const [soundEnabled, setSoundEnabled] = useState(true); // 🔊 Bật/tắt SFX
    ...
    // Hàm bọc âm thanh để tôn trọng cài đặt soundEnabled
    ...
    }, [answered, currentQuestion, gameState, playSound, sounds]);
    ...
    const handleOptionClick = (index) => {
        ...
        if (isCorrect) {
            // 🔊 Âm thanh đúng (có biến thể cho streak cao)
    ...
    }, [currentIndex, totalQuestions, clearTimer, playSound, sounds]);
    ...
    }, [clearTimer, enterFullscreen, shuffleQuestions, playSound, sounds]);
    ...
    if (gameState === 'start') {
        return (
            <div
                ref={containerRef}
                className={`min-h-screen bg-[#f7f9ff] flex flex-col items-center justify-center px-6 py-12 overflow-y-auto ${isFullscreen ? 'fixed inset-0 z-[9999] bg-[#f7f9ff]' : ''
                    }`}
            >
                ...
                <div className="absolute top-4 right-4 flex items-center gap-2 z-50">
                    ...
                    >
                        <span className="material-symbols-outlined text-2xl text-[#8b5000]">tune</span>
                    </button>
                </div>

                {showVolumeControl && (
                    <div className="absolute top-16 right-4 bg-white rounded-xl p-4 shadow-lg border border-[#d1e4fb] z-50 min-w-[220px]">
    ...
    return (
        <div
            ref={containerRef}
            className={`min-h-screen bg-[#f7f9ff] flex flex-col px-4 sm:px-6 pt-4 pb-8 md:pt-6 overflow-y-auto ${isFullscreen ? 'fixed inset-0 z-[9999] bg-[#f7f9ff]' : ''
                }`}
        >
            <div id="youtube-background-player" style={{ position: 'fixed', width: '0', height: '0', opacity: 0, pointerEvents: 'none' }} />
            <div id="youtube-countdown-player" style={{ position: 'fixed', width: '0', height: '0', opacity: 0, pointerEvents: 'none' }} />
            ...
            <div className="w-full max-w-3xl mx-auto flex-1 flex flex-col gap-4">
                ...
                <div className="w-full h-5 bg-[#d1e4fb] rounded-full overflow-hidden shadow-inner flex-shrink-0">
                    <div
                        className={`h-full rounded-full transition-all duration-100 flex flex-col justify-start ${timeLeft <= 3 ? 'bg-[#ba1a1a]' : timeLeft <= 5 ? 'bg-[#ff9800]' : 'bg-[#63c664]'
                            }`}
                ...
                <div className="flex justify-center">
                    <div className={`text-2xl font-bold ${timeLeft <= 3 ? 'text-[#ba1a1a] animate-pulse' : timeLeft <= 5 ? 'text-[#ff9800]' : 'text-[#8b5000]'
                        }`}>
                        ⏱️ {timeLeft}s
                    </div>
                </div>
            ...
            {showResultBox && (
                ...
                >
                    <div
                        className={`bg-white rounded-3xl p-8 md:p-12 max-w-md w-full mx-4 text-center shadow-2xl transform transition-all duration-300 scale-100 border-t-8 cursor-default ${resultType === 'correct' ? 'border-[#006e1c]' : 'border-[#ba1a1a]'
                            }`}
